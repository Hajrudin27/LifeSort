...
| `lifesort-food-v2` | A,D | AsyncStorage / Z2 (APP-050; was Z1) | monthlyBudgetByMonth, purchases[], pantryItems[] (structured/legacy), shoppingItems[], offers[], recipes[] (typed ingredients), standardPrices[], globalStandardPrices[], globalOffers[], savedPlans, selectedStores[] | APP-047 v0→v1 keeps historical recipe amounts verbatim; APP-050 v1→v2 keeps historical Pantry quantity text verbatim as `legacyQuantityText`, preserves optional expiry and metadata, and infers no dates, amounts or family (see [app-050-pantry-inventory.md](./app-050-pantry-inventory.md)) | `store/useFoodStore.ts` | versioned; core/storage/migrations/foodIngredients.ts | User sweep |
...
| filesystem:document-directory/lifesort-backup-json | `documentDirectory/lifesort-backup-<date>.json`, filesystem | B | `{version:1,exportedAt,data}` since 49c4355; `version:2` since APP-040 (Economy money in MinorUnits); `version:3` since APP-042 (explicit expense recurrence cadence and day anchor); `version:4` since APP-047 (typed recipe ingredients); `version:5` since APP-050 (typed Pantry and explicit dates); user-triggered archive | `utils/shared/dataBackup.ts` / `backupValidation.ts`; never startup hydrated | External export/import contract; APP-097 | Existing logout does not sweep these exports; unchanged audit finding |
...
| `food/5c85adc-v0.json` | Food | 5c85adc, the last pre-APP-047 build | Zustand 0, `{name, amount}` ingredients: two seed copies, a user recipe, a fetched recipe | Zustand 2 (APP-047 typed ingredients, APP-050 Pantry text retained as `legacyQuantityText`) | Synthetic recipes, Pantry, prices and plans; seed text is bundled app content |
