...
| **The device itself** | Local-first primary storage | Everything in §1 marked `local` | AsyncStorage (plaintext), SecureStore (keychain/keystore), app document directory. |
...
| Display name | `account` | Greeting and personalisation | local+cloud | Supabase | `profiles` row; cascade on deletion; local copy cleared on logout | personal | Contact Info › Name | Personal info › Name |
| Age | `account` | Collected at sign-up; no feature reads it today — see §6-D1 | local+cloud | Supabase | `profiles` row; cascade | personal | Contact Info › Other User Contact Info | Personal info › Other info |
| Gender | `account` | Currently gates visibility of the cycle tab — see §6-D2 | local+cloud | Supabase | `profiles` row; cascade | personal | Sensitive Info | Personal info › Other info |
...
| Uploaded files (expense and warranty evidence) | shared | Receipts and documents attached to an expense or a warranty | local+cloud | Supabase | Storage bucket `attachments`; row in `attachments` cascades and files are removed before deletion, with a daily server sweep for orphans. **Local copies in the app document directory are not deleted on logout — §6-D4** | document | User Content › Photos or Videos | Photos and videos › Photos; Files and docs |
| Trip attachments | `travel` | Boarding passes and booking documents attached to a trip | local | – | Local file in the app document directory only. **Never uploaded** — trips are absent from `AttachmentOwnerType`, so these files do not survive a reinstall and never reach the `attachments` table — §6-D15 | document | Not collected (device-local) | Not collected (device-local) |
...
| Menstrual cycle dates | `cycle` | Cycle overview and predictions | local+cloud | Supabase | `cycles`; cascade; local store cleared on logout. **Stored unencrypted in AsyncStorage — §6-D3** | health | Health & Fitness › Health | Health and fitness › Health info |
| Symptom logs and free-text notes | `cycle` | Symptom history and patterns | local+cloud | Supabase | `symptom_logs`; cascade. **Unencrypted locally — §6-D3** | health | Health & Fitness › Health | Health and fitness › Health info |
| Cycle settings and reminder preference | `cycle` | Prediction parameters and reminder timing | local+cloud | Supabase | `cycle_settings`; cascade | health | Health & Fitness › Health | Health and fitness › Health info |
| Reviewed health content | `cycle` | Reference articles about conditions and symptoms | cloud | Supabase | `health_conditions`, `symptom_glossary` — editorial content, shared read-only, not user data | ordinary | Not collected | Not collected |
...
| Home layout | `core-shell` | Which cards are pinned or hidden, and when each module was last opened | local | – | `lifesort-home-layout` in AsyncStorage; never leaves the device and is not synced | ordinary | Not collected (device-local) | Not collected (device-local) |
...
| `@react-native-async-storage/async-storage` | 2.2.0 | React Native Community | Local persistence for every Zustand store | All `local` and `local+cloud` data in §1, in plaintext | No | Apple required-reason API `UserDefaults` (CA92.1), already declared |
| `expo-secure-store` | ~57.0.3 | Expo | Keychain/keystore for the auth session, PIN hash and lockout counter | Session token, PIN hash | No | None — device-local credential storage |
| `expo-crypto` | ~57.0.2 | Expo | CSPRNG salt for PIN derivation | PIN salt | No | None |
...
| `expo-linking` | ~57.0.9 | Expo | Deep-link URL handling (transitive dependency of `expo-router`) | Deep-link URLs | No | None |
...
| --- | --- | --- | --- |
| D1 | Age is collected at sign-up but no feature reads it. | `store/useAuthStore.ts` `signUp(email, password, name, age, gender)` writes `age` to `profiles`; no read site exists outside the profile screen itself. Collecting a birth-year-equivalent without a purpose fails data minimisation. | APP-017 |
| D2 | Gender is collected and used as a feature gate. | `app/(tabs)/_layout.tsx` hides the cycle tab unless `gender === 'female'`. Gender is a special-category-adjacent field being used where an explicit module choice belongs. | APP-020, APP-071 |
| D3 | Health data is persisted unencrypted. | `store/useCycleStore.ts` persists cycles, symptom logs and free-text notes to plaintext AsyncStorage under `lifesort-cycle`. A device backup or a forensic read exposes special-category data. | APP-028 |
| D4 | Attachment files survive logout on disk. | `utils/shared/attachmentStorage.ts` copies files to `documentDirectory/attachments/`; `utils/auth/clearAllLocalData.ts` clears stores and the signed-URL cache but never deletes that directory. On a shared device the next user's filesystem still holds the previous user's receipts. | APP-029, APP-021 |
| D5 | Scheduled notifications are not cancelled on logout. | Reminders are cancelled per entity in `utils/*/​*Reminder.ts`, but nothing cancels pending notifications during sign-out. A cycle reminder can fire after logout — the exact leak §9.2 of the specification warns about. | APP-021, APP-073 |
...
| D9 | Six dependencies ship without being used. | `expo-device`, `expo-web-browser`, `expo-widgets`, `react-native-android-widget`, `@expo/ui` and `expo-dev-client` have no import in `app/`, `store/`, `utils/`, `components/`, `hooks/` or `lib/`. `expo-device` alone adds a required-reason API declaration to the privacy manifest for a capability the app does not use. | APP-115, APP-159 |
