...
| D4 | ~~Attachment files survive logout on disk.~~ **Closed by APP-021.** `clearAttachmentCache()` deletes the directory on sign-out and before a different account signs in. Encrypting the cache while it exists is still open. | APP-029 |
