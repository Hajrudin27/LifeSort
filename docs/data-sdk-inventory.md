...
| D14 | Most entity IDs still use `Date.now()` plus `Math.random()` or `Date.now().toString()`. | APP-029 moved attachment cache filenames to opaque CSPRNG names because filenames are part of the sensitive document cache. Record/entity IDs remain unchanged and still need the APP-030 platform ID primitive. | APP-030 |
