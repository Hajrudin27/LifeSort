...
- **Financial views share a read model.** Economy totals and projections use a common read model with integration tests. See the [financial read-model audit](docs/app-039-financial-read-model.md).
- **Recurring costs state their cadence.** A recurring expense carries `monthly`, `quarterly` or `yearly` explicitly along with the day of month it is anchored to, so occurrences are generated with calendar arithmetic (no 30-day months, no impossible dates) and a short month never moves the schedule. The boundary for future bank-derived recurring candidates only ever suggests — confirming one creates no second financial fact. See [recurring bills](docs/app-042-recurring-bills.md) and [ADR-0035](docs/adr/0035-manual-recurrence-is-an-explicit-frequency-and-bank-candidates-only-suggest.md).
- **Economy money is integer minor units.** Economy amounts are DKK øre as safe integers on the device. Only amounts proven to round-trip the actual PostgreSQL `numeric` → PostgREST JSON transport without any material sub-cent alias are persisted, legacy floats migrate without rounding, and one localized formatter renders every amount exactly. Food and Travel amounts are not yet migrated. See [the money primitive](docs/app-040-money.md) and [ADR-0034](docs/adr/0034-economy-money-is-integer-minor-units-on-the-client-and-numeric-on-the-server.md).
