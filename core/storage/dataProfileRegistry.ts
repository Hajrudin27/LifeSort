...
export const DATA_DOMAINS = [
  ...
  {
    id: 'core.sync-status',
    title: 'Sync status presentation',
    module: 'core-shell',
    description: 'Ephemeral account-scoped outbox projection. Legacy device-only success timestamp key is never read or written and remains registered for logout cleanup.',
...
export const PERSISTENCE_SURFACES = [
  ...
  surface('async-storage:lifesort-expenses', 'async-storage', 'Encrypted Zustand key lifesort-expenses', 'device', ['economy.expenses', 'economy.attachments'], ['store/useExpensesStore.ts', 'core/storage/documentCacheStorage.ts'], { kind: 'external', currentVersion: 2, owner: 'core/storage/documentCacheStorage.ts', reason: 'Specialized AES-GCM v1 adapter owns plaintext legacy upgrade, key access and protected failure handling, and runs the APP-040 inner v0 to v1 money and APP-042 v1 to v2 recurrence definition in memory before one encrypted commit.' }),
  surface('async-storage:lifesort-income-v2', 'async-storage', 'Zustand key lifesort-income-v2', 'device', ['economy.income'], ['store/useIncomeStore.ts'], { kind: 'versioned', owner: 'core/storage/migrations/economyMoney.ts', currentVersion: 1, reason: 'APP-040: v0 major-unit income upgrades once to DKK MinorUnits before hydration; unsupported precision fails closed.' }),
  ...
  surface('async-storage:lifesort-food-v2', 'async-storage', 'Zustand key lifesort-food-v2', 'device', ['food.user-grocery-finance', 'food.user-planning', 'food.seed-recipes', 'food.global-catalogue'], ['store/useFoodStore.ts'], { kind: 'versioned', owner: 'core/storage/migrations/foodIngredients.ts', currentVersion: 1, reason: 'APP-047: v0 recipe ingredients {name, amount} are kept verbatim as typed legacy ingredients before hydration; no family or quantity is inferred and unknown shapes fail closed.' }),
