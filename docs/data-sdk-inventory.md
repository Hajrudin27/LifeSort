...
| Uploaded files (receipt photos, documents) | shared | Evidence attached to an expense, warranty or trip | local+cloud | Supabase | Storage bucket `attachments`; row in `attachments` cascades and files are removed before deletion, with a daily server sweep for orphans. **Local copies in the app document directory are not deleted on logout — §6-D4** | document | User Content › Photos or Videos | Photos and videos › Photos; Files and docs |
