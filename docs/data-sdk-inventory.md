...
| Pantry, shopping list, recipes, meal plans | `food` | Meal planning and waste reduction | local+cloud | Supabase | `food_pantry_items` (optional explicit purchase/opening/expiry dates and quantity/unit), `food_shopping_items`, `food_recipes`, `food_saved_plans`; cascade. Seed recipes are not user data and survive logout | ordinary | User Content › Other User Content | App activity › Other user-generated content |
