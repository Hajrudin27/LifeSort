...
| Gender | `account` | Optional profile field, asked in onboarding rather than at sign-up (APP-017). Still gates the cycle tab — see §6-D2 | local+cloud | Supabase | `profiles` row; cascade | personal | Sensitive Info | Personal info › Other info |
...
| D2 | Gender is still used as a feature gate. | APP-017 removed it from sign-up, so it is now optional and asked in onboarding. But `app/(tabs)/_layout.tsx` still hides the cycle tab unless `gender === 'female'`, now combined with the module choice. Replacing the gate outright is APP-071. | APP-020, APP-071 |
