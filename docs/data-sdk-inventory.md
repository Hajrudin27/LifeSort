...
| Pantry, shopping list, recipes, meal plans | `food` | Meal planning and waste reduction | local+cloud | Supabase | `food_pantry_items` (optional explicit purchase/opening/expiry dates and quantity/unit), `food_shopping_items` with child `food_shopping_item_derivations` (week, current amount and recipe-ID provenance), `food_recipes`, `food_saved_plans`; cascade. Seed recipes are not user data and survive logout | ordinary | User Content › Other User Content | App activity › Other user-generated content |
...
| Standalone private documents | `documents` | Keeping personal documents in one private place | local+cloud | Supabase | Storage bucket `documents` (private, 25 MiB ceiling) plus a row in `documents`. The row cascades from `auth.users`; objects are removed by the client before account deletion. `orphaned_document_paths` exists as a service-role backstop for objects whose owning account is gone, but no scheduler for it is established here. Only metadata is kept on the device, encrypted; bytes are never cached locally and are read through short-lived signed URLs — APP-055 | document | User Content › Other User Content | Files and docs |
...
| --- | --- | --- |
| Sign out | Local domain stores are reset, both signed-URL caches (attachments and documents) are cleared and the PIN is deleted. Language, theme mode and seed recipes deliberately survive. Locally cached attachment files and pending notifications do **not** — §6-D4, §6-D5 | `core/auth/clearLocalUserData.ts` |
...
| Account deletion | Storage objects are removed first from **both** the `attachments` and `documents` buckets (while the session may still touch them), then `delete_my_account` removes the account and every table cascades from `auth.users`. `orphaned_attachment_paths` is swept daily under the existing APP-022 operational contract; `orphaned_document_paths` exists as the same kind of service-role backstop but **no scheduler for it is established here** (APP-055). Each stage is reported to the user, and what is retained is disclosed on the screen — see [`docs/account-deletion.md`](./account-deletion.md) | `core/auth/deleteAccount.ts` |
| Signed download URL | Expires after 1 hour and is cached in memory only | `utils/shared/attachmentSync.ts`, `core/documents/documentSync.ts` |
